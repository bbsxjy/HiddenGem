...
def parse_bool_env(env_var: str, default: bool = False) -> bool:
    ...
    if value_lower in true_values:
        ...
        # 如果无法识别，记录警告并返回默认值
        print(f" 无法解析环境变量 {env_var}='{value}'，使用默认值 {default}")
        return default
...
def parse_int_env(env_var: str, default: int = 0) -> int:
    ...
    try:
        ...
        print(f" 无法解析环境变量 {env_var}='{value}' 为整数，使用默认值 {default}")
        return default
...
def parse_float_env(env_var: str, default: float = 0.0) -> float:
    ...
    try:
        ...
        print(f" 无法解析环境变量 {env_var}='{value}' 为浮点数，使用默认值 {default}")
        return default
...
def parse_list_env(env_var: str, separator: str = ",", default: Optional[list] = None) -> list:
    ...
    try:
        ...
        print(f" 无法解析环境变量 {env_var}='{value}' 为列表，使用默认值 {default}")
        return default
