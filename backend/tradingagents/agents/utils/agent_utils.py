    ...
    def get_china_market_overview(
        ...
        try:
            ...
            if not adapter.provider or not adapter.provider.connected:
                # 如果Tushare不可用，返回基础信息
                logger.warning(f" Tushare不可用，返回基础市场概览")
...
"""
    ...
    def get_stock_fundamentals_unified(
        ...
        """
        #  会话级缓存检查
        cache_key = f"fundamentals_{ticker}_{start_date}_{end_date}_{curr_date}"
        if cache_key in Toolkit._session_cache:
            logger.info(f" [会话缓存] 命中缓存: {cache_key}")
            logger.debug(f" [会话缓存] 返回缓存数据，长度: {len(Toolkit._session_cache[cache_key])} 字符")
            return Toolkit._session_cache[cache_key]
        else:
            logger.debug(f" [会话缓存] 未命中缓存: {cache_key}")
            ...
            if is_china:
                ...
                try:
                    ...
                    if hk_data and len(hk_data) > 100 and "" not in hk_data:
                        ...
                        logger.warning(f" [统一基本面工具] 港股主要数据源质量不佳")

                except Exception as e:
                    logger.error(f" [统一基本面工具] 港股主要数据源失败: {e}")
...
**数据源**: {hk_info.get('source', '基础信息')}
 ...
 注意：详细的价格和财务数据暂时无法获取，建议稍后重试或使用其他数据源。
...
"""
                        ...
                        logger.warning(f" [统一基本面工具] 港股使用最终备用方案")
...
"""
            ...
            #  存入会话缓存
            Toolkit._session_cache[cache_key] = combined_result
            logger.debug(f" [会话缓存] 已缓存结果: {cache_key}")
            ...
            #  错误结果也缓存（避免重复失败）
            Toolkit._session_cache[cache_key] = error_msg
            logger.debug(f" [会话缓存] 已缓存错误结果: {cache_key}")
