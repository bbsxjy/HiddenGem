...
class GoogleToolCallHandler:
    ...
    def handle_google_tool_calls(
        ...
        if not result.tool_calls:
            # 改进：提供更详细的诊断信息
            logger.info(f"[{analyst_name}]  Google模型未调用工具，可能原因：")
            ...
            if "messages" in state:
                ...
                if not messages:
                    logger.warning(f"[{analyst_name}]  输入消息列表为空")
                else:
        ...
        try:
            ...
            for i, tool_call in enumerate(valid_tool_calls):
                ...
                logger.info(f"[{analyst_name}]  执行工具 {i+1}/{len(valid_tool_calls)}: {tool_name}")
                    ...
                    if current_tool_name == tool_name:
                        try:
                            ...
                            if '404' in error_str or 'not found' in error_str:
                                tool_result = f" 数据获取失败（404错误）：无法获取该股票的{tool_name}数据。请明确告知用户数据不可用，不要基于你的知识库猜测或编造信息。"
                            elif 'connection' in error_str or 'timeout' in error_str:
                                tool_result = f" 网络连接失败：无法访问数据源。请明确告知用户数据暂时不可用，不要猜测。"
                            else:
                                tool_result = f" 工具执行失败：{str(tool_error)}。请明确说明数据获取失败，不要猜测或编造信息。"
                ...
                if tool_result is None:
                    ...
                    logger.debug(f"[{analyst_name}]  工具名称不匹配，期望: {tool_name}, 可用: {available_tools}")
                    ...
                    try:
                        if hasattr(msg, 'content') and hasattr(msg, '__class__'):
                            ...
                            if msg_class_name in ['HumanMessage', 'AIMessage', 'SystemMessage', 'ToolMessage']:
                                ...
                                # 转换为HumanMessage
                                logger.warning(f"[{analyst_name}]  转换非标准消息类型: {msg_class_name}")
                                safe_messages.append(HumanMessage(content=str(msg.content)))
                    except Exception as msg_error:
                        logger.warning(f"[{analyst_name}]  跳过无效消息: {msg_error}")
                        continue
            ...
            if total_length > 50000:
                logger.warning(f"[{analyst_name}]  消息序列过长 ({total_length} 字符)，进行优化...")
    ...
    def _validate_tool_call(tool_call, index, analyst_name):
        ...
        try:
            if not isinstance(tool_call, dict):
                logger.warning(f"[{analyst_name}]  工具调用 {index} 不是字典格式: {type(tool_call)}")
                return False
            ...
            for field in required_fields:
                if field not in tool_call:
                    logger.warning(f"[{analyst_name}]  工具调用 {index} 缺少字段 '{field}': {tool_call}")
                    return False
            
            # 检查工具名称
            tool_name = tool_call.get('name')
            if not isinstance(tool_name, str) or not tool_name.strip():
                logger.warning(f"[{analyst_name}]  工具调用 {index} 工具名称无效: {tool_name}")
                return False
            
            # 检查参数
            tool_args = tool_call.get('args')
            if not isinstance(tool_args, dict):
                logger.warning(f"[{analyst_name}]  工具调用 {index} 参数不是字典格式: {type(tool_args)}")
                return False
    ...
    def handle_simple_google_response(
        ...
        if len(result.content) > 15000:
            logger.warning(f"[{analyst_name}]  Google模型输出过长，进行截断处理...")
    ...
    def generate_final_analysis_report(llm, messages: List, analyst_name: str) -> str:
        ...
        """
        if not GoogleToolCallHandler.is_google_model(llm):
            logger.warning(f" [{analyst_name}] 非Google模型，跳过Google工具处理器")
            return ""
            ...
            try:
                ...
                if hasattr(result, 'content'):
                    ...
                    if not content or len(content.strip()) == 0:
                        logger.warning(f"[{analyst_name}]  Google模型返回内容为空 (尝试 {attempt + 1}/{max_retries})")
                        ...
                        if attempt < max_retries - 1:
                            ...
                            logger.warning(f"[{analyst_name}]  Google模型最终分析报告生成失败 - 所有重试均返回空内容")
                            # 使用降级报告
    ...
    def _generate_fallback_report(messages: List, analyst_name: str) -> str:
        ...
        # 提取工具结果
        tool_results = []
        all_tools_failed = True  # 默认假设所有工具都失败
        has_valid_data = False   # 是否有有效数据
        ...
        for msg in messages:
            if isinstance(msg, ToolMessage) and hasattr(msg, 'content'):
                ...
                # 检查是否为失败消息
                if not any(keyword in content for keyword in ['', '失败', '错误', 'Error', '404', 'not found']):
                    # 检查是否有实际数据（不只是空字符串或错误消息）
                    if len(content.strip()) > 100:  # 至少100字符才认为有有效数据
                        has_valid_data = True
                        all_tools_failed = False

        # 🆕 如果所有工具都失败或没有有效数据，明确告知
        if all_tools_failed or not has_valid_data:
            error_report = f"""## {analyst_name}分析报告
...
"""
            ...
            for i, result in enumerate(tool_results):
                if '' in result or '失败' in result:
                    ...
                    error_report += f"- 工具 {i+1}: {error_line}\n"
...
1. 检查股票代码是否正确
2. 稍后重试（数据源可能暂时不可用）
3. 使用其他分析师的报告（如技术分析、基本面分析）

**重要提示**: 本报告不包含任何猜测或编造的信息，所有内容基于工具调用的实际结果。
"""
            logger.warning(f"[{analyst_name}]  生成数据获取失败报告（所有工具失败或无有效数据）")
            return error_report
        ...
        if tool_results:
            ...
            for content in tool_results:
                ...
                # 只添加非错误的结果
                if not any(keyword in content for keyword in ['', '失败', 'Error']):
                    valid_results.append(content)

            if valid_results:
                tool_summary = "\n\n".join([f"**数据来源 {i+1}:**\n{result}" for i, result in enumerate(valid_results)])
                report = f"""## {analyst_name}数据汇总报告
...
由于模型响应异常，此为基于工具数据的简化报告。上述数据来自实际数据源，未包含任何猜测或编造的信息。

建议结合其他分析师的报告进行综合判断。
"""
                logger.info(f"[{analyst_name}]  生成简化数据汇总报告（有{len(valid_results)}个有效数据源）")
                return report

        # 如果没有任何有效结果
        report = f"""## {analyst_name}分析报告

###  无可用数据

分析完成，但未能获取到有效的数据。建议检查数据源或重新尝试分析。

**重要**: 本报告不包含任何猜测或编造的信息。
"""
        logger.warning(f"[{analyst_name}]  生成无数据可用报告")
        return report
