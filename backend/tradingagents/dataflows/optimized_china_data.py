...
class OptimizedChinaDataProvider:
    ...
    def _generate_fundamentals_report(self, symbol: str, stock_data: str) -> str:
        ...
        if any("（估算值）" in str(v) for v in financial_estimates.values() if isinstance(v, str)):
            data_source_note = "\n **数据说明**: 部分财务指标为估算值，建议结合最新财报数据进行分析"
...
"""
    ...
    def _estimate_financial_metrics(self, symbol: str, current_price: str) -> dict:
        ...
        logger.warning(f" 无法获取真实财务数据，使用估算数据: {symbol}")
    ...
    def _get_real_financial_metrics(self, symbol: str, price_value: float) -> dict:
        ...
        try:
            ...
            if tushare_provider.connected:
                # 获取Tushare财务数据
                financial_data = tushare_provider.get_financial_data(symbol)

                if financial_data:
                    logger.info(f" Tushare财务数据获取成功: {symbol}")
                    # 获取股票基本信息
                    stock_info = tushare_provider.get_stock_info(symbol)

                    # 解析Tushare财务数据
                    logger.debug(f" 调用Tushare解析函数，股价: {price_value}")
                    metrics = self._parse_financial_data(financial_data, stock_info, price_value)
                    logger.debug(f" Tushare解析结果: {metrics}")

                    if metrics:
                        logger.info(f" [数据源] Tushare官方数据解析成功")
                        metrics['data_source'] = 'Tushare'
                        return metrics
                    else:
                        logger.warning(f" Tushare数据解析失败，尝试备用数据源")
                else:
                    logger.warning(f" Tushare未获取到{symbol}财务数据，尝试备用数据源")
            else:
                logger.warning(f" Tushare未连接（请检查token配置），尝试备用数据源")
            ...
            if akshare_provider.connected:
                ...
                if financial_data and any(not v.empty if hasattr(v, 'empty') else bool(v) for v in financial_data.values()):
                    ...
                    if metrics:
                        logger.info(f" [数据源] AKShare备用数据解析成功")
                        metrics['data_source'] = 'AKShare'
                        return metrics
                    else:
                        logger.warning(f" AKShare解析失败")
                else:
                    logger.warning(f" AKShare未获取到{symbol}财务数据")
            else:
                logger.warning(f" AKShare未连接")

        except Exception as e:
            logger.error(f" 获取{symbol}真实财务数据失败: {e}")

        logger.error(f" 所有数据源均失败，无法获取{symbol}的财务数据")
        return None
