...
class TushareProvider:
    ...
    def __init__(self, token: str = None, enable_cache: bool = True):
        ...
        if not token:
            logger.warning(" 未找到Tushare API token，请设置TUSHARE_TOKEN环境变量")
            return
        ...
        try:
            ...
            if data is not None:
                ...
                if not data.empty:
                    ...
                    logger.warning(f" [Tushare详细日志] 返回的DataFrame为空")
            else:
                logger.warning(f" [Tushare详细日志] 返回数据为None")
            ...
            if data is not None and not data.empty:
                ...
                if self.enable_cache and self.cache_manager:
                    try:
                        ...
                        logger.error(f" 缓存保存失败: {cache_error}")
                        logger.error(f" [Tushare详细日志] 缓存异常类型: {type(cache_error).__name__}")

                logger.info(f" [Tushare详细日志] get_stock_daily 执行成功，返回数据")
                return data
            else:
                logger.warning(f" Tushare返回空数据: {ts_code}")
                logger.warning(f" [Tushare详细日志] 空数据详情: data={data}, empty={data.empty if data is not None else 'N/A'}")
                return pd.DataFrame()
    ...
    def _calculate_forward_adjusted_prices(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if data.empty or 'pct_chg' not in data.columns:
            logger.warning(" 数据为空或缺少pct_chg列，无法计算前复权价格")
            return data
    ...
    def get_index_daily(self, symbol: str, start_date: str = None, end_date: str = None) -> pd.DataFrame:
        ...
        Args:
            symbol: 指数代码（如：000001.SH上证指数, 399001.SZ深证成指）
        ...
        """
        if not self.connected:
            logger.error(" Tushare未连接，无法获取指数数据")
            return pd.DataFrame()

        try:
            # 标准化指数代码
            ts_code = self._normalize_symbol(symbol)
            logger.info(f" [指数数据] 获取{ts_code}指数数据...")
            ...
            if data is not None and not data.empty:
                logger.info(f" [指数数据] 获取到{len(data)}条数据，列名: {list(data.columns)}")
                ...
                # 标准化列名（与股票数据保持一致）
                # 指数数据列：ts_code, trade_date, close, open, high, low, pre_close, change, pct_chg, vol, amount
                # 这些列名与股票数据的列名一致，无需重命名

                logger.info(f" 获取指数{ts_code}数据成功: {len(data)}条")
                return data
            else:
                logger.warning(f" Tushare返回空指数数据: {ts_code}")
                return pd.DataFrame()
...
def get_china_stock_data_tushare(symbol: str, start_date: str = None, end_date: str = None) -> pd.DataFrame:
    """
    获取中国股票/指数数据的便捷函数（Tushare数据源）
    自动识别股票和指数代码，使用相应的API接口
    ...
    # 判断是否为指数代码
    if StockUtils.is_china_index(symbol):
        logger.info(f" [Tushare] 识别为指数代码: {symbol}，使用index_daily接口")
        return provider.get_index_daily(symbol, start_date, end_date)
    else:
        logger.info(f" [Tushare] 识别为股票代码: {symbol}，使用daily接口")
