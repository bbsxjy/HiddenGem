...
**重要：您必须只返回纯JSON，不要任何额外的文字、解释或markdown格式。**

请从提供的分析报告中提取以下信息，并**严格按照以下JSON格式返回**：
...
4. reasoning应该是简洁的中文摘要（不超过200字）
5. 所有内容必须使用中文，不允许任何英文投资建议

特别注意：
- 股票代码 {stock_symbol or '未知'} 是{market_info['market_name']}，使用{currency}计价
- 目标价格必须与股票的交易货币一致（{currency_symbol}）
- **只返回JSON对象，不要包含任何额外的解释、前言、后记或markdown标记（如```json）**
- **如果报告中没有明确的目标价格，请根据建议方向估算一个合理的价格**

如果某些信息在报告中没有明确提及，请使用合理的默认值。

示例正确格式：
{{"action": "卖出", "target_price": 190.0, "confidence": 0.95, "risk_score": 0.85, "reasoning": "财务数据显示风险较高，建议立即卖出"}}""",
            ),
            ("human", full_signal),
        ]
        ...
        try:
            ...
            logger.debug(f" [SignalProcessor] LLM原始响应: {response[:300]}...")
            ...
            if json_match:
                ...
                try:
                    ...
                    logger.error(f" [SignalProcessor] JSON解析失败: {e}, JSON文本: {json_text[:200]}")
                    # 如果第一次解析失败，尝试更宽松的提取
                    logger.info(f" [SignalProcessor] 尝试更宽松的JSON提取...")
                    # 直接尝试解析整个清理后的响应
                    try:
                        decision_data = json.loads(cleaned_response)
                    except json.JSONDecodeError:
                        logger.error(f" [SignalProcessor] 所有JSON解析尝试失败，使用简单提取")
